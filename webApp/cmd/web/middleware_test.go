...
func Test_application_addIPToContext(t *testing.T) {
	tests := []struct{
		...
		// we can ignore what's in the address field
		emptyAddr bool
	}{
		{"", "", "", false},
		{"", "", "", true},
	...
	for _, e := range tests {
		...
		//we call remote address to get it from the request and we don't get anything back.
		if e.emptyAddr {
			req.RemoteAddr = ""
		}
...
func Test_application_ipFromContext(t *testing.T) {
	// crate an app var of type application

	// get a context
	ctx := context.Background()

	// pu something in the context
